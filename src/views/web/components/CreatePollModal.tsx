...
const CreatePollModal: React.FC<CreatePollModalProps> = ({ roomId, onClose, onSuccess }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50" onClick={onClose}>
            <div 
                className="bg-white rounded-xl shadow-xl w-[400px] overflow-hidden flex flex-col"
                onClick={(e) => e.stopPropagation()}
            >
                <div className="px-4 py-3 border-b border-gray-200 flex justify-between items-center bg-[#f5f5f5]">
                ...
                <div className="p-4 flex-1 overflow-y-auto max-h-[60vh] styled-scrollbar text-sm">
                    <div className="mb-4">
                        <input 
                            ...
                            className="w-full text-base font-medium border-b border-gray-300 focus:border-blue-500 pb-2 focus:outline-none placeholder-gray-400 text-gray-800"
                            value={question}
                    ...
                    <div className="space-y-3 mb-4">
                        {options.map((opt, idx) => (
                            <div key={idx} className="flex flex-col relative">
                                <div className="flex items-center group border border-gray-200 rounded-lg focus-within:border-blue-500 bg-white">
                                    <input 
                                        placeholder={`Lựa chọn ${idx + 1}`}
                                        className="flex-1 w-full p-3 font-normal text-gray-700 outline-none rounded-lg bg-transparent"
                                        value={opt}
                    ...
                    <div className="space-y-3 pt-3 border-t border-gray-100 mt-2">
                        <h4 className="text-gray-800 font-semibold mb-2">Cài đặt</h4>
                        <div className="flex items-center justify-between">
                            <span className="text-gray-700">Chọn nhiều phương án</span>
                            <input 
                                type="checkbox" 
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-gray-700">Cho phép thêm phương án</span>
                            <input 
                                type="checkbox" 
                ...
                <div className="px-4 py-3 bg-white border-t border-gray-100 flex justify-end gap-3">
                    <button 
                        className="px-5 py-2 text-sm text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200 transition-colors font-medium"
                        onClick={onClose}
                    >
                        Hủy
                    </button>
