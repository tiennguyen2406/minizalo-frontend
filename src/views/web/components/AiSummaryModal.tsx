...
const AiSummaryModal: React.FC<AiSummaryModalProps> = ({ roomId, onClose }) => {
    ...
    const fetchHistory = async () => {
        ...
        try {
            ...
            toast.error("Không thể tải lịch sử tóm tắt");
        } finally {
            ...
            if (isMainHeading) {
                return (
                    <div key={index} className="mt-6 mb-3 border-l-4 border-blue-500 pl-4">
                        <h4 className="text-blue-600 font-bold text-lg leading-7">{renderedContent}</h4>
                    </div>
                );
            }
            if (isBullet) {
                    ...
                    <div className="flex items-center gap-2">
                        {view === 'create' && (
                            <button
                                ...
                                className="flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-blue-600 hover:bg-blue-50 rounded-lg transition-all"
                            >
                    ...
                    <>
                        ...
                        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50 shrink-0">
                            ...
                            {!summary && (
                                ...
                                >
                                    {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : <Flashlight className="w-5 h-5" />}
                                    ...
                                    >
                                        <div className="flex items-center justify-between mb-4">
                                            <div className="flex items-center gap-2 text-gray-400">
                                                ...
                                                <span className="text-xs font-medium">{formatTime(item.ttl - (5 * 24 * 60 * 60))}</span>
                                            </div>
                                            <button 
                                                onClick={() => copyText(item.content)}
                                                className="opacity-0 group-hover:opacity-100 p-1.5 hover:bg-blue-50 text-blue-600 rounded-md transition-all"
                                                title="Sao chép"
                                            >
                                                <Copy className="w-4 h-4" />
                                            </button>
