...
const ChatWindow: React.FC<ChatWindowProps> = ({ roomId }) => {
    ...
    const { messages, addMessage, setCurrentRoom, setMessages, rooms, prependMessages } = useChatStore();
    ...
    const loadMoreHistory = useCallback(async () => {
        ...
        try {
            const result = await chatService.getChatHistory(roomId, 20, historyLastKey);
            setHistoryLastKey(result.lastEvaluatedKey ?? null);
            setHistoryHasMore(!!result.lastEvaluatedKey);
            const sorted = (result.messages || [])
                .slice()
                ...
                <div className="h-16 flex items-center px-4 shrink-0 justify-between" style={{ borderBottom: '1px solid var(--border-primary)', backgroundColor: 'var(--bg-primary)', boxShadow: 'var(--shadow-sm)', transition: 'background-color 0.3s ease' }}>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                        <button
                            ...
                            className={`w-9 h-9 flex items-center justify-center rounded-full transition-colors ${searchOpen ? 'bg-blue-100 text-blue-600' : 'hover:bg-gray-100 text-gray-500'
                                }`}
                        >
