...
/** Chuỗi giống số điện thoại (để gợi ý tìm bạn / kết bạn). */
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1 }}>
        ...
        />
        <div
          style={{ height: 10, background: bg, borderRadius: 6, width: "80%" }}
        />
      </div>
    </div>
  );
...
const GlobalSearchBar: React.FC<GlobalSearchBarProps> = ({ onSelectRoom }) => {
  ...
  const localResults = useMemo((): LocalSearchResults => {
    ...
    const matchedFiles = allMessages.filter(
      (m) =>
        ["IMAGE", "VIDEO", "FILE", "DOCUMENT"].includes(m.type) && !m.isDeleted,
    );
  ...
  const handleSelectContact = useCallback(
    async (contact: UserProfile) => {
      ...
      // Mở chat 1-1 ngay tại tab Tin nhắn (giao diện giống Zalo sau khi tìm SĐT).
      try {
        const store = useChatStore.getState();
        const newRoom = await store.createPrivateRoom(contact.id);
        const patched = {
          ...newRoom,
        ...
        store.upsertRoom(patched);
        store.setMessages(newRoom.id, []);
        store.setPendingOpenDirectInfoRoomId(newRoom.id);
        onSelectRoom(newRoom.id);
        handleClose();
      } catch (e) {
        console.error("Open direct chat from search failed", e);
        const q = query.trim();
        if (q) {
          router.push(
            `/(tabs)/contacts?phoneSearch=${encodeURIComponent(q)}` as any,
          );
        } else {
  ...
  const tabStyle = (tab: SearchTab): React.CSSProperties => ({
    ...
    border: "none",
    borderBottom:
      activeTab === tab ? "2px solid #0068ff" : "2px solid transparent",
    cursor: "pointer",
            ...
            >
              {(
                [
                  ...
                  [
                    "contacts",
                    `Liên hệ${totalContacts ? ` (${totalContacts})` : ""}`,
                  ],
                  [
                    "messages",
                    `Tin nhắn${totalMessages ? ` (${totalMessages})` : ""}`,
                  ],
            ...
            <div style={{ overflowY: "auto", flex: 1 }}>
              ...
              {showContacts && totalContacts > 0 && (
                <>
                  ...
                  {/* Tài khoản tìm được qua API (/users/search — gồm SĐT) */}
                  {mergedContacts.profiles.length > 0 &&
                    isPhoneLikeQuery(query) && (
                      <div
                        style={{
                  ...
                  {mergedContacts.profiles.slice(0, 8).map((c) => {
                    const hasDirectRoom = rooms.some(
                      (r) =>
                        r.type === "PRIVATE" &&
                        r.participants?.some((p) => p.id === c.id),
                    );
                    return (
                      <div
                        key={c.id}
                          ...
                          <div style={{ fontSize: 12, color: textMuted }}>
                            {c.phone ? (
                              <>
                                ...
                                >
                                  <HighlightText text={c.phone} query={query} />
                                </span>
                              </>
                            ) : null}
                            {!hasDirectRoom && (
                            ...
                            {hasDirectRoom && !c.phone && (
                              <span>Nhấn để mở hội thoại</span>
                            )}
                          </div>
                        </div>
                      </div>
              ...
              {showFiles && totalFiles > 0 && (
                <>
                  ...
                  {mergedFiles.slice(0, 5).map((msg) => {
                    const room = rooms.find((r) => r.id === msg.roomId);
                    const name =
                      msg.fileName || msg.attachments?.[0]?.name || "File";
