...
import FriendshipWelcomeCard from "./FriendshipWelcomeCard";
import { Message, User } from "@/shared/types";
import { useChatStore } from "@/shared/store/useChatStore";
import { useAuthStore } from "@/shared/store/authStore";
import LazyImage from "./LazyImage";

/** Đồng bộ với ChatWindow — nền danh sách tin */
...
interface MessageListProps {
  ...
  /** Gửi nhanh emoji/sticker từ thẻ chào mừng kết bạn. */
  onWelcomeStickerPick?: (emoji: string) => void;
}
  ...
  if (isFailed) {
    return (
      <span
        ...
        title="Gửi thất bại — nhấp để thử lại"
      >
        ⚠️
      </span>
    );
    ...
    return (
      <span title={`Đã xem (${readCount})`}>
        ...
        >
          ...
          <polyline
            points="20 6 9 17"
            style={{ opacity: 0.5, transform: "translateX(-4px)" }}
          />
        </svg>
      </span>
    );
        ...
        if (
          ...
          Math.abs(
            new Date(next.createdAt).getTime() -
              new Date(group[group.length - 1].createdAt).getTime(),
          ) <= IMAGE_GROUP_THRESHOLD_MS
        ) {
          group.push(next);
          j++;
        } else {
...
const MessageList: React.FC<MessageListProps> = ({
  ...
  const friendshipWelcomeByRoomId = useChatStore(
    (s) => s.friendshipWelcomeByRoomId,
  );
  const clearFriendshipWelcome = useChatStore((s) => s.clearFriendshipWelcome);
  const { user: authUser } = useAuthStore();

  const welcomePartnerName = roomId
    ? friendshipWelcomeByRoomId[roomId]
    : undefined;
  const partnerUser =
    welcomePartnerName && participants.length
  ...
  // Show skeleton on first load (no messages yet) — trừ khi đang hiện thẻ chào kết bạn
  if (isInitialLoad && messages.length === 0 && !welcomePartnerName) {
    return (
      <div
        style={{
          flex: 1,
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col justify-end min-h-full px-4 pt-4 pb-2">
        {welcomePartnerName && (
          <FriendshipWelcomeCard
            partnerName={welcomePartnerName}
            ...
            onPickSticker={(emoji) => {
              if (roomId) clearFriendshipWelcome(roomId);
              onWelcomeStickerPick?.(emoji);
            }}
          />
        )}
        {messages.length === 0 && !welcomePartnerName ? (
          ...
          buildRenderItems(messages).map((item) => {
            if (item.type === "imageGroup") {
              ...
              const isFirstInGroup =
                !messages[item.startIndex - 1] ||
                messages[item.startIndex - 1].senderId !== firstMsg.senderId;
              const nextAfterGroup = messages[item.startIndex + group.length];
              const isLastInGroup =
                !nextAfterGroup ||
                nextAfterGroup.senderId !== firstMsg.senderId;
