            ...
            <div className="flex flex-col justify-end min-h-full px-4 pt-4 pb-2">
                ...
                {messages.length === 0 ? (
                    ...
                    buildRenderItems(messages).map((item) => {
                        ...
                        // ─── Render PIN_NOTIFICATION / SYSTEM messages ───
                        if (msg.type === 'PIN_NOTIFICATION' || (msg.senderId === 'system' && msg.type !== 'SYSTEM')) {
                            // "đã ghim" xuất hiện trong cả "đã ghim" và "đã bỏ ghim" → dùng điều kiện chặt hơn
                            const isPinAction = !!msg.content && msg.content.includes('đã ghim') && !msg.content.includes('bỏ ghim');
                            const pinnedMsgId = msg.replyToId;
                            return (
                                <div
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <svg
                                            ...
                                            height="14"
                                            viewBox="0 0 24 24"
                                            fill={isPinAction ? '#f59e0b' : 'none'}
                                            stroke={isPinAction ? '#f59e0b' : '#9ca3af'}
                                            strokeWidth="2"
                                            strokeLinecap="round"
                                        ...
                                        <span style={{ fontSize: 12.5, color: '#6b7280', flexShrink: 1 }}>
                                            {msg.content}
                                        </span>
                                            ...
                                            <button
                                                onClick={() => {
                                                    ...
                                                    if (el) {
                                                        el.scrollIntoView({ behavior: 'smooth', block: 'center' });
                                                        el.style.transition = 'background-color 0.5s ease';
                                                        el.style.backgroundColor = 'rgba(253, 224, 71, 0.4)';
                                                        setTimeout(() => { el.style.backgroundColor = 'transparent'; }, 2000);
                                                    } else {
                                                        // Element chưa render (virtual scroll) - thử fetch history thêm
                                                        console.warn('[Pin] Pinned message not in DOM:', pinnedMsgId);
                                                    }
                                                }}
                                                style={{
                                                    background: 'none',
